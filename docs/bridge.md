...
Two replicas may hold one park store, and then the delete is the claim. A `Resume` stands the instance up and then deletes the row; the delete says how many rows it removed, and only the replica whose delete removed one keeps the instance. When two replicas race the same token they both stand a copy up, but exactly one delete removes the row, so exactly one replica wins. The loser removed nothing, breaks the copy it just built, and answers `NOT_FOUND`, the same answer a spent token gets, because a token another replica already claimed is spent from where the loser stands. A lying hash still fails before any of this: the resume that reads the row is refused for a hash that disagrees, and the store is never touched, so wrong hash stays ABORTED and a lost race stays NOT_FOUND with no way to confuse the two.
