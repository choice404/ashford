...
- **A richer query surface.** `Store.query` reads by equality or by comparisons joined with `&&`, ordered by one column and bounded to a count when asked, and `Store.count` and `Store.sum` answer the size and the total of a match without the rows. Disjunction, negation, the other aggregates, average, min, max, and grouping, an offset, and arbitrary SQL in the language are later steps; a set conjunction cannot name is composed in the pledge from several reads.
