...
- **A richer query surface.** `Store.query` reads by equality or by comparisons joined with `&&`, ordered by one column when asked. Disjunction, negation, aggregation, a row limit, and arbitrary SQL in the language are later steps; a set conjunction cannot name is composed in the pledge from several reads.
