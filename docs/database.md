...
| `Store.count(S, predicate)` | how many rows match, counted by the store | `Result<Int, StoreError>` |
| `Store.sum(S, column, predicate)` | the matching rows' column summed by the store | `Result<Int or Float, StoreError>` |
| `Store.min(S, column, predicate)` | the smallest matching value, `None` over no rows | `Result<Option<T>, StoreError>` |
| `Store.max(S, column, predicate)` | the largest matching value, `None` over no rows | `Result<Option<T>, StoreError>` |
| `Store.avg(S, column, predicate)` | the mean of a numeric column, `None` over no rows | `Result<Option<Float>, StoreError>` |
...
A row is a `Record` whose fields are the schema's columns in declaration order, so `find` returns `Some(row)` or `None` and the write forms return `Unit`. `Store.query` reads sets two ways. The column and value form binds one column by equality. The predicate form takes comparisons, `==`, `!=`, `<`, `<=`, `>`, `>=`, joined with `&&` and `||`, `&&` binding tighter as everywhere in the language: the left side of every comparison is a bare column name resolved against the schema at compile time, never a variable, the right side is an ordinary expression checked against that column's declared type, and the whole predicate lowers onto one prepared statement whose terms are all positionally bound, a disjunction normalized at compile time into an or of and groups so the statement's shape is decided before the program runs. One predicate grammar holds everywhere: `Store.query`, `Store.count`, and `Store.sum` all take the same comparisons, the same `&&`, the same `||`, and the same `!`. Negation never reaches the database: the compiler pushes every `!` down through the connectives and flips the comparison at the leaf, `!(balance < lo)` becoming `balance >= lo`, so what runs is always a plain or of and groups and the runtime does not know negation exists. The flips are sound in the store's own terms: every row the language writes carries every column, and for a NULL that reaches a table some other way, SQLite's comparisons exclude the row from both a predicate and its flipped complement alike, so the rewrite never changes which rows answer. A trailing `asc(column)` or `desc(column)` orders the matching rows by one column before they come back, the column a bare name resolved against the schema at compile time like every other; `asc` and `desc` are reserved spellings in that position and nowhere else. A trailing `limit(count)` after the order bounds the answer to at most `count` rows: the count is an ordinary `Int` expression bound as a parameter like every value, a negative count is refused at runtime rather than read as unbounded, and a bound without an order is refused at compile time, because the rows a bound cuts are defined only once an order says which ones come first. In every form no rows is `Ok` of an empty list, never an error. `Store.count` takes the same predicate and answers how many rows match without materializing any of them: the count is computed where the rows live, so a pledge that only needs the number never pays for the list. `Store.sum` does the same for one numeric column, an `Int` column answering `Int` and a `Float` column answering `Float`, any other column type refused at compile time; a sum over no rows is the zero of the column's own type, never a missing value, because an empty ledger owes nothing rather than owing a question. `Store.min` and `Store.max` take any scalar column, since every scalar column compares, and `Store.avg` takes a numeric one and always answers `Float`; all three answer an `Option`, `Some` of the value or `None` over no rows, because an empty set has no smallest member and the surface says so rather than inventing one. Sum has a zero to stand on; the extremes and the mean do not, and the two answers differ on purpose. A join across schemas, or a set conjunction cannot name, is composed in the pledge from several reads. The connection every operation uses is the current instance's, resolved from the pledge's own context, so a store operation never names a connection and never reaches another instance's. Every value bound into an operation crosses as a parameter, positionally bound in the prepared statement, never concatenated into text, so a string column holding `'; drop table` is a string and nothing else. Rows read out are decoded onto the instance the way every result is, so a row lives on the instance heap and dies at the instance's break, one lifetime rule from the language surface down to the database driver.
...
Because a transaction is one episode and not a latch, a pledge in a transactional subcontract is fulfilled at most once. A second call to a pledge whose transaction has already committed or rolled back is `GEAS_ERR_STATE`, delivered through the wait like every fulfillment error. This is a deliberate departure from the loose pledge latch, where a later call runs and returns but never moves the state; a committed transaction is closed, there is nothing left to join, and pretending otherwise would be the dishonest kind of convenient.
...
`sign` opens the connection to the bound `dsn`, reconciles every declared schema with the live database, and locks the vows. A connection that will not open, a `dsn` that will not parse, or a schema that will not reconcile fails the sign, the first two with `GEAS_ERR_STORE` and the last with `GEAS_ERR_TYPE`, and nothing is left half open. Fulfilling a pledge runs it against the connection, inside a transaction when the pledge belongs to a transactional subcontract and in autocommit otherwise. `break` rolls back any open transaction, closes the connection, and reclaims the instance's heap, in that order, so no uncommitted write survives a break and no connection outlives its instance. A partially fulfilled contract reports exactly which pledges landed and which broke, and a transactional subcontract in that report is all committed or all rolled back, never half.

## The parked instance

An instance's durable state fits in one row, and the runtime will write it there and stand it back up. `geas_instance_park(c, dsn, key)` puts the contract's name, version, and shape hash, the lifecycle state and the signing time, every vow value, every pledge latch with the `Err` payload it carries, and the fate of every transactional episode into the runtime's own `geas_park` table in the database behind `dsn`, created on first use, one `INSERT OR REPLACE` per key. The value blobs are the canonical value encoding, the codec `test-wire` pins byte for byte, so the park format inherits the codec's goldens. Parking is a write, not an ending: the caller still holds a live signature.

`geas_instance_resume(rt, dsn, key, expected_hash, out)` reads the row back and signs its state up against the current runtime: the contract is found by the recorded name, the vows decode onto the new instance, the latches and payloads replay, a store-backed contract reopens its `dsn` vow and reconciles its schemas exactly as sign does, and the recorded state and signing time land unchanged. The recorded version and shape hash must match the registered module's, and a nonzero `expected_hash` must agree too, the same skew rule sign runs; a key nobody parked, or a contract this runtime does not register, is `GEAS_ERR_NAME`.

A park is a state between walks, never a snapshot of one mid flight. An unwaited future, an open transactional episode, or an instance the caller already ended with an explicit break each refuse with `GEAS_ERR_STATE`; an automatically broken instance parks with its errors readable, because keeping them readable is what the automatic break is for. A resumed `TXN_DONE` stays done, so a committed episode can never run twice, no matter how many times the instance crosses the table.
...
`GEAS_ERR_STORE` is the backend failing to do what it was told: a connection lost mid fulfillment, a disk with no room, a commit a constraint refused, a `dsn` that would not open. It rides back through the future the host already waits on, the same channel every fulfillment error uses, so a host's store error handling has one place to live. It is deliberately narrow. A contract's own rules, an overdrawn account, an unknown id, a duplicate the contract means to reject, are the contract's errors, returned as values in the contract's own error type, `PaymentError` and its variants, not folded into `GEAS_ERR_STORE`. The status is for the store failing the runtime, not for the contract failing its business. A `CHECK` constraint the schema did not ask for is the backend's refusal and is `GEAS_ERR_STORE`; a balance the pledge checks and rejects is `Err(Insufficient)` and is the contract working.

## Concurrency

One connection lives on one instance, opened at sign, and the instance's own lock already serializes every fulfillment against it, so a connection is never touched by two threads at once and the runtime adds no new lock for the store. Two instances against one database are two connections, and the backend arbitrates between them: SQLite's file locking, or its write ahead log when the `dsn` asks for it, decides who writes when, and a writer that must wait surfaces as `GEAS_ERR_STORE` rather than a silent stall past the backend's busy timeout. Isolation between connections is the backend's to define and this layer does not weaken or strengthen it; a contract that needs a stronger isolation than the default asks the backend for it in the `dsn`. The worker pool runs store pledges the way it runs any pledge, and because the connection sits under the instance lock the pool needs no store awareness.
...
- **NoSQL and key value stores.** v1 is relational: a schema is a table and a row is flat. A document or key value backend behind the same `GeasStore` vtable, with its own schema shape, is a later layer.
...
- **A richer query surface.** `Store.query` reads by comparisons joined with `&&` and `||`, ordered by one column and bounded to a count when asked, and `Store.count` and `Store.sum` answer the size and the total of the same predicates without the rows. Grouping, an offset, and arbitrary SQL in the language are later steps; a set the predicates cannot name is composed in the pledge from several reads.
