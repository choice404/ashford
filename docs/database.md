...
- **A richer query surface.** `Store.query` reads by comparisons joined with `&&` and `||`, ordered by one column and bounded to a count when asked, and `Store.count` and `Store.sum` answer the size and the total of the same predicates without the rows. The other aggregates, average, min, max, and grouping, an offset, and arbitrary SQL in the language are later steps; a set the predicates cannot name is composed in the pledge from several reads.
