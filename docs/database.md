...
- **A richer query surface.** `Store.query` reads by equality or by comparisons joined with `&&`. Ordering, disjunction, negation, aggregation, and arbitrary SQL in the language are later steps; a set conjunction cannot name is composed in the pledge from several reads.
