...
// compiler knows, so Store.find, Store.insert, Store.update, and Store.delete
// name a schema declared in the same contract and lower onto the runtime
// primitives that carry a row across the boundary. A row is the schema's
...
//
// Store.query, the bound predicate form, is not in this release; it lands with
// the transactional surface. Every operation returns a Result so a store
// failure is a value in the surface's own error type. The backend failing the
// runtime, a connection lost, a disk with no room, a constraint the backend
// refused, rides back as the ASH_ERR_STORE status through the wait the host
// already reads, never folded into a contract's own error; the contract's own
// rules, an unknown id, an overdraft, stay values in the contract's error type
