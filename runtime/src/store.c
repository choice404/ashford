/* store.c: the SQLite driver behind the GeasStore vtable, docs/database.md's
 ...
 * self contained: it leans on SQLite and the GeasValue layout in geas_abi.h and
 ...
 * row is flat, so a parameter or a column that is not a scalar is GEAS_ERR_TYPE
 ...
 * GeasStoreAlloc, the same discipline the wire decoder kept over its owner: the
 ...
 * GEAS_ERR_STORE, the store failing the runtime, never the contract failing its
 ...
 * flat row, so it is refused here and reported GEAS_ERR_TYPE by every caller. */
...
static GeasStatus bind_one(sqlite3_stmt* stmt, int idx, const GeasValue* v) {
    switch (v->ty) {
    case GEAS_TY_INT:
        return sqlite3_bind_int64(stmt, idx, v->as.i) == SQLITE_OK
                   ? GEAS_OK : GEAS_ERR_STORE;
    case GEAS_TY_UINT:
    ...
    case GEAS_TY_STRING: {
        ...
        int rc = sqlite3_bind_blob(stmt, idx, p, (int)v->as.s.len,
                                   SQLITE_TRANSIENT);
        return rc == SQLITE_OK ? GEAS_OK : GEAS_ERR_STORE;
    }
    default:
 ...
 * placeholders is the caller building the wrong call, GEAS_ERR_STORE; a non
 * scalar value is GEAS_ERR_TYPE, and the first of either stops the bind. */
static GeasStatus bind_params(sqlite3_stmt* stmt, const GeasValue* params,
                             size_t nparams) {
    if (sqlite3_bind_parameter_count(stmt) != (int)nparams) return GEAS_ERR_STORE;
    for (size_t i = 0; i < nparams; i++) {
        GeasStatus st = bind_one(stmt, (int)i + 1, &params[i]);
 ...
 * that is not a scalar is GEAS_ERR_TYPE; an allocation the alloc refuses is
 * GEAS_ERR_OOM. */
static GeasStatus read_row(sqlite3_stmt* stmt, const uint32_t* col_types,
                          size_t ncols, const GeasStoreAlloc* alloc,
                          GeasValue* rec_out) {
    GeasValue* fields = NULL;
    if (ncols) {
        fields = (GeasValue*)alloc->bytes(alloc->ctx, ncols * sizeof(GeasValue));
        if (!fields) return GEAS_ERR_OOM;
        memset(fields, 0, ncols * sizeof(GeasValue));
    }
    for (size_t i = 0; i < ncols; i++) {
        uint32_t ty = col_types[i];
        if (!tag_scalar(ty)) return GEAS_ERR_TYPE;
        GeasValue* f = &fields[i];
        f->ty = ty;
        f->tag = 0;
        switch (ty) {
        case GEAS_TY_INT:
        ...
        default:
            return GEAS_ERR_TYPE; /* unreachable past tag_scalar */
        }
    }
    memset(rec_out, 0, sizeof(*rec_out));
    rec_out->ty = GEAS_TY_RECORD;
...
GeasStatus geas_store_open(const char* dsn, GeasStore** out) {
    ...
    *out = NULL;
    if (!dsn) return GEAS_ERR_STORE;

    GeasStore* s = (GeasStore*)sqlite3_malloc((int)sizeof(GeasStore));
    if (!s) return GEAS_ERR_OOM;
    s->db = NULL;
 ...
 * flat row refuses; the caller reports GEAS_ERR_TYPE on the NULL. */
 ...
 * underscore with a non digit lead. A schema and its columns are geas
 ...
 * compile time here. */
static GeasStatus store_create_table(GeasStore* s, const GeasSchemaDesc* schema) {
    ...
    for (uint32_t i = 0; i < schema->ncols; i++) {
        ...
        if (!ct) return GEAS_ERR_TYPE;
        if (!store_ident_ok(schema->cols[i].name)) return GEAS_ERR_STORE;
        need += strlen(schema->cols[i].name) + strlen(ct) + 24;
    }
    char* sql = (char*)sqlite3_malloc((int)need);
    if (!sql) return GEAS_ERR_OOM;
    size_t n = 0;
    ...
    GeasStatus st = geas_store_exec(s, sql);
    sqlite3_free(sql);
    return st;
}
 ...
 * A divergence is GEAS_ERR_TYPE, the storage version of the shape hash, the same
 ...
 * interpolated. */
static GeasStatus store_validate_table(GeasStore* s, const GeasSchemaDesc* schema) {
    char pragma[128 + 64];
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        if (i >= schema->ncols) { st = GEAS_ERR_TYPE; break; }
    ...
    sqlite3_finalize(stmt);
    if (st != GEAS_OK) return st;
    if (rc != SQLITE_DONE && rc != SQLITE_ROW) return GEAS_ERR_STORE;
    if (i != schema->ncols) return GEAS_ERR_TYPE;
    return GEAS_OK;
}

GeasStatus geas_store_reconcile(GeasStore* s, const GeasSchemaDesc* schema) {
    if (!s || !s->db || !schema || !schema->table) return GEAS_ERR_STORE;
    if (schema->ncols == 0) return GEAS_ERR_TYPE;
    if (!store_ident_ok(schema->table)) return GEAS_ERR_STORE;
...
GeasStatus geas_store_exec(GeasStore* s, const char* sql) {
    if (!s || !s->db || !sql) return GEAS_ERR_STORE;
    char* err = NULL;
...
GeasStatus geas_store_exec_params(GeasStore* s, const char* sql,
    ...
    GeasStatus st = bind_params(stmt, params, nparams);
    if (st != GEAS_OK) {
        sqlite3_finalize(stmt);
        return st;
    }
    ...
    sqlite3_finalize(stmt);
    if (rc != SQLITE_DONE) return GEAS_ERR_STORE;
...
GeasStatus geas_store_query(GeasStore* s, const char* sql,
                          ...
                          const GeasString* col_names, size_t ncols,
    ...
    if (!rows_out) return GEAS_ERR_TYPE;
    memset(rows_out, 0, sizeof(*rows_out));
    if (!s || !s->db || !sql || !alloc || !alloc->bytes) return GEAS_ERR_STORE;
    if (ncols && !col_types) return GEAS_ERR_TYPE;
    for (size_t i = 0; i < ncols; i++) {
        if (!tag_scalar(col_types[i])) return GEAS_ERR_TYPE;
    }
    ...
    GeasValue* backbone = NULL;
    if (n) {
        backbone = (GeasValue*)alloc->bytes(alloc->ctx, n * sizeof(GeasValue));
        if (!backbone) {
            sqlite3_finalize(stmt);
            return GEAS_ERR_OOM;
        }
        memset(backbone, 0, (size_t)n * sizeof(GeasValue));
    }
    uint64_t j = 0;
 ...
 * constraint blocks, a begin over an open transaction, is GEAS_ERR_STORE. */
GeasStatus geas_store_begin(GeasStore* s) {
    return geas_store_exec(s, "BEGIN");
}

GeasStatus geas_store_commit(GeasStore* s) {
    return geas_store_exec(s, "COMMIT");
}

GeasStatus geas_store_rollback(GeasStore* s) {
    return geas_store_exec(s, "ROLLBACK");
}
