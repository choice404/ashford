 ...
 * it turns frame headers and GeasValues into little endian bytes and back. The
 ...
 * GEAS_ERR_TYPE, the same word a thunk uses for a shape it cannot accept; an
 * oversized payload or a failed allocation is GEAS_ERR_OOM, the refusal the
 ...
 * GEAS_WIRE_MAX_DEPTH on both sides. */
...
GeasStatus geas_wire_frame_write(uint32_t kind, uint64_t request_id,
    ...
    if (kind < GEAS_WIRE_HELLO || kind > GEAS_WIRE_ERROR) return GEAS_ERR_TYPE;
    if (payload_len > GEAS_WIRE_MAX_PAYLOAD) return GEAS_ERR_OOM;
    memcpy(out, GEAS_WIRE_MAGIC, 4);
    put_u32(out + 4, kind);
...
GeasStatus geas_wire_frame_read(const uint8_t in[GEAS_WIRE_HEADER_LEN],
                              GeasWireFrame* out) {
    if (!in || !out) return GEAS_ERR_TYPE;
    if (memcmp(in, GEAS_WIRE_MAGIC, 4) != 0) return GEAS_ERR_TYPE;
...
static int tag_crosses(uint32_t ty) {
    return ty <= GEAS_TY_SUM && ty != GEAS_TY_PLEDGE_REF;
}
...
static GeasStatus size_add(size_t* total, uint64_t n) {
    if (n > SIZE_MAX - *total) return GEAS_ERR_OOM;
    *total += (size_t)n;
    return GEAS_OK;
}

static GeasStatus encode_size(const GeasValue* v, uint32_t depth,
                             size_t* total) {
    if (depth > GEAS_WIRE_MAX_DEPTH) return GEAS_ERR_TYPE;
    GeasStatus st = size_add(total, 8); /* the u32 ty, u32 tag header */
    if (st != GEAS_OK) return st;

    switch ((GeasTypeTag)v->ty) {
    case GEAS_TY_UNIT:
        return v->tag == 0 ? GEAS_OK : GEAS_ERR_TYPE;
    case GEAS_TY_INT:
    case GEAS_TY_UINT:
    case GEAS_TY_FLOAT:
        return v->tag == 0 ? size_add(total, 8) : GEAS_ERR_TYPE;
    case GEAS_TY_BOOL:
        if (v->tag != 0 || v->as.b > 1) return GEAS_ERR_TYPE;
        return size_add(total, 1);
    case GEAS_TY_BYTE:
        return v->tag == 0 ? size_add(total, 1) : GEAS_ERR_TYPE;
    case GEAS_TY_CHAR:
    ...
    case GEAS_TY_STRING:
        if (v->tag != 0) return GEAS_ERR_TYPE;
        if (v->as.s.len && !v->as.s.ptr) return GEAS_ERR_TYPE;
        if ((st = size_add(total, 8)) != GEAS_OK) return st;
        return size_add(total, v->as.s.len);
    case GEAS_TY_LIST: {
        if (v->tag != 0) return GEAS_ERR_TYPE;
        if (!tag_crosses(v->as.list.elem_ty)) return GEAS_ERR_TYPE;
        if (v->as.list.len && !v->as.list.data) return GEAS_ERR_TYPE;
        if ((st = size_add(total, 12)) != GEAS_OK) return st;
        const GeasValue* e = (const GeasValue*)v->as.list.data;
        for (uint64_t i = 0; i < v->as.list.len; i++) {
            if (e[i].ty != v->as.list.elem_ty) return GEAS_ERR_TYPE;
            if ((st = encode_size(&e[i], depth + 1, total)) != GEAS_OK)
                return st;
        }
        return GEAS_OK;
    }
    case GEAS_TY_MAP: {
        if (v->tag != 0) return GEAS_ERR_TYPE;
        if (!tag_keyable(v->as.list.elem_ty)) return GEAS_ERR_TYPE;
        if (v->as.list.len % 2) return GEAS_ERR_TYPE;
        if (v->as.list.len && !v->as.list.data) return GEAS_ERR_TYPE;
        if ((st = size_add(total, 12)) != GEAS_OK) return st;
        const GeasValue* e = (const GeasValue*)v->as.list.data;
        for (uint64_t i = 0; i < v->as.list.len; i++) {
    ...
    case GEAS_TY_RECORD:
    case GEAS_TY_SUM: {
        if (v->ty != GEAS_TY_SUM && v->tag != 0) return GEAS_ERR_TYPE;
        if (v->as.list.len && !v->as.list.data) return GEAS_ERR_TYPE;
        if ((st = size_add(total, 8)) != GEAS_OK) return st;
        const GeasValue* e = (const GeasValue*)v->as.list.data;
        for (uint64_t i = 0; i < v->as.list.len; i++) {
    ...
    case GEAS_TY_OPTION:
        if (v->tag > 1) return GEAS_ERR_TYPE;
        if (v->tag == 0) return v->as.box ? GEAS_ERR_TYPE : GEAS_OK;
        if (!v->as.box) return GEAS_ERR_TYPE;
        return encode_size((const GeasValue*)v->as.box, depth + 1, total);
    case GEAS_TY_RESULT:
        if (v->tag > 1 || !v->as.box) return GEAS_ERR_TYPE;
        return encode_size((const GeasValue*)v->as.box, depth + 1, total);
    default:
        /* GEAS_TY_INSTANCE, GEAS_TY_PLEDGE_REF, and anything unknown. */
        return GEAS_ERR_TYPE;
    }
}
...
static void encode_write(const GeasValue* v, uint8_t** p) {
    put_u32(*p, v->ty);
    put_u32(*p + 4, v->tag);
    *p += 8;
    ...
    case GEAS_TY_OPTION:
        if (v->tag == 1) encode_write((const GeasValue*)v->as.box, p);
        return;
    case GEAS_TY_RESULT:
        encode_write((const GeasValue*)v->as.box, p);
        return;
    default:
...
GeasStatus geas_wire_encode_value(const GeasValue* v, uint8_t* buf, size_t cap,
    ...
    size_t total = 0;
    GeasStatus st = encode_size(v, 1, &total);
    if (st != GEAS_OK) return st;
    *need = total;
    if (!buf || cap < total) return GEAS_ERR_OOM;
    uint8_t* p = buf;
    encode_write(v, &p);
    return GEAS_OK;
}
...
static GeasStatus decode_rec(GeasContract* owner, WireCur* c, uint32_t depth,
                            GeasValue* out) {
    if (depth > GEAS_WIRE_MAX_DEPTH) return GEAS_ERR_TYPE;
    const uint8_t* h;
    if (!take(c, 8, &h)) return GEAS_ERR_TYPE;
    ...
    switch ((GeasTypeTag)ty) {
    case GEAS_TY_UNIT:
        return tag == 0 ? GEAS_OK : GEAS_ERR_TYPE;
    case GEAS_TY_INT:
        if (tag != 0 || !take(c, 8, &h)) return GEAS_ERR_TYPE;
    ...
    case GEAS_TY_BOOL:
        if (tag != 0 || !take(c, 1, &h)) return GEAS_ERR_TYPE;
        if (*h > 1) return GEAS_ERR_TYPE;
        out->as.b = *h;
        return GEAS_OK;
    case GEAS_TY_BYTE:
        if (tag != 0 || !take(c, 1, &h)) return GEAS_ERR_TYPE;
        ...
        return GEAS_OK;
    case GEAS_TY_CHAR: {
        if (tag != 0 || !take(c, 4, &h)) return GEAS_ERR_TYPE;
        uint32_t ch = get_u32(h);
        if (ch > 0x10FFFF || (ch >= 0xD800 && ch <= 0xDFFF))
            return GEAS_ERR_TYPE; /* not a Unicode scalar value */
        out->as.ch = ch;
        return GEAS_OK;
    }
    case GEAS_TY_STRING: {
        ...
        if (len > c->left) return GEAS_ERR_TYPE;
        const uint8_t* bytes;
        take(c, (size_t)len, &bytes);
        *out = geas_string_copy(owner, bytes, len);
        if (len && !out->as.s.ptr) return GEAS_ERR_OOM;
        return GEAS_OK;
    }
    case GEAS_TY_LIST: {
        if (tag != 0 || !take(c, 12, &h)) return GEAS_ERR_TYPE;
        uint64_t count = get_u64(h);
        uint32_t elem_ty = get_u32(h + 8);
        if (!tag_crosses(elem_ty)) return GEAS_ERR_TYPE;
        if (!count_plausible(c, count)) return GEAS_ERR_TYPE;
        GeasStatus st = geas_list_new(owner, elem_ty, count, out);
        if (st != GEAS_OK) return st;
        for (uint64_t i = 0; i < count; i++) {
            GeasValue elem;
            if ((st = decode_rec(owner, c, depth + 1, &elem)) != GEAS_OK)
                return st;
            if (elem.ty != elem_ty) return GEAS_ERR_TYPE;
            if ((st = geas_list_push(owner, out, &elem)) != GEAS_OK) return st;
        }
        return GEAS_OK;
    }
    case GEAS_TY_TUPLE: {
        ...
        if (!count_plausible(c, arity)) return GEAS_ERR_TYPE;
        GeasStatus st = geas_tuple_new(owner, arity, out);
        if (st != GEAS_OK) return st;
        GeasValue* slots = (GeasValue*)out->as.list.data;
        for (uint64_t i = 0; i < arity; i++) {
            if ((st = decode_rec(owner, c, depth + 1, &slots[i])) != GEAS_OK)
                return st;
        }
        return GEAS_OK;
    }
    case GEAS_TY_RECORD:
    case GEAS_TY_SUM: {
        if (ty == GEAS_TY_RECORD && tag != 0) return GEAS_ERR_TYPE;
        if (!take(c, 8, &h)) return GEAS_ERR_TYPE;
        uint64_t count = get_u64(h);
        if (!count_plausible(c, count)) return GEAS_ERR_TYPE;
        GeasValue* fields = NULL;
        if (count) {
            fields = (GeasValue*)geas_bytes(owner,
                                          count * sizeof(GeasValue));
            if (!fields) return GEAS_ERR_OOM;
            memset(fields, 0, (size_t)count * sizeof(GeasValue));
        }
        out->as.list.data = fields;
        out->as.list.len = count;
        out->as.list.cap = count;
        for (uint64_t i = 0; i < count; i++) {
            GeasStatus st = decode_rec(owner, c, depth + 1, &fields[i]);
            if (st != GEAS_OK) return st;
        }
        return GEAS_OK;
    }
    case GEAS_TY_MAP: {
        ...
        if (slots % 2) return GEAS_ERR_TYPE;
        if (!tag_keyable(key_ty)) return GEAS_ERR_TYPE;
        if (!count_plausible(c, slots)) return GEAS_ERR_TYPE;
        *out = geas_map_new(owner, key_ty);
        GeasValue* data = NULL;
        if (slots) {
            data = (GeasValue*)geas_bytes(owner, slots * sizeof(GeasValue));
            if (!data) return GEAS_ERR_OOM;
            memset(data, 0, (size_t)slots * sizeof(GeasValue));
        }
        out->as.list.data = data;
        out->as.list.len = slots;
        out->as.list.cap = slots;
        for (uint64_t i = 0; i < slots; i++) {
            GeasStatus st = decode_rec(owner, c, depth + 1, &data[i]);
            if (st != GEAS_OK) return st;
            if (i % 2 == 0) {
                if (data[i].ty != key_ty) return GEAS_ERR_TYPE;
                ...
                for (uint64_t j = 0; j < i; j += 2) {
                    if (geas_value_eq(&data[j], &data[i]))
                        return GEAS_ERR_TYPE;
                }
            }
        }
    ...
    case GEAS_TY_OPTION: {
        if (tag > 1) return GEAS_ERR_TYPE;
        if (tag == 0) return GEAS_OK; /* None: eight bytes of header only */
        GeasValue* box = geas_box(owner);
    ...
    default:
        /* GEAS_TY_INSTANCE, GEAS_TY_PLEDGE_REF, and anything unknown never
...
GeasStatus geas_wire_decode_value(GeasContract* owner, const uint8_t* buf,
    ...
    if (!owner || !out) return GEAS_ERR_TYPE;
    if (!buf && len) return GEAS_ERR_TYPE;
    WireCur c = { buf, len };
    GeasStatus st = decode_rec(owner, &c, 1, out);
    if (st != GEAS_OK) {
        memset(out, 0, sizeof(*out));
        return st;
    }
