 ...
 * module and drives every pledge of skeleton/lang.geas from C, asserting the
 ...
 * out of bounds index, which the ABI pins as GEAS_ERR_TYPE from the thunk.
...
static int map_pair_is(const GeasValue* m, uint64_t i, const char* key,
                       int64_t val) {
    if (m->ty != GEAS_TY_MAP || m->as.list.len < (i + 1) * 2) return 0;
    const GeasValue* e = (const GeasValue*)m->as.list.data;
    const GeasValue* k = &e[i * 2];
    const GeasValue* v = &e[i * 2 + 1];
    if (k->ty != GEAS_TY_STRING || k->as.s.len != strlen(key)) return 0;
    if (memcmp(k->as.s.ptr, key, k->as.s.len) != 0) return 0;
    return v->ty == GEAS_TY_INT && v->as.i == val;
}
...
static int result_int(const GeasValue* v, uint32_t tag, int64_t want) {
    if (v->ty != GEAS_TY_RESULT || v->tag != tag || !v->as.box) return 0;
    const GeasValue* p = (const GeasValue*)v->as.box;
    return p->ty == GEAS_TY_INT && p->as.i == want;
}

static int result_bool(const GeasValue* v, uint32_t tag, int want) {
    if (v->ty != GEAS_TY_RESULT || v->tag != tag || !v->as.box) return 0;
    const GeasValue* p = (const GeasValue*)v->as.box;
    return p->ty == GEAS_TY_BOOL && p->as.b == (want ? 1 : 0);
}

static int result_str(const GeasValue* v, uint32_t tag, const char* want) {
    if (v->ty != GEAS_TY_RESULT || v->tag != tag || !v->as.box) return 0;
    const GeasValue* p = (const GeasValue*)v->as.box;
...
static GeasValue run1(GeasContract* c, const char* name, GeasValue arg,
                     const char* what) {
    GeasValue out;
    memset(&out, 0, sizeof(out));
    CHECK(geas_pledge_fulfill_sync(c, name, &arg, 1, NULL, 0, &out) == GEAS_OK,
          what);
    return out;
}
...
int main(void) {
    ...
    CHECK(geas_module_load(rt, "target/geas-out/liblang.geas.so") == GEAS_OK,
          "module load");

    GeasContract* c = NULL;
    CHECK(geas_contract_sign(rt, "LangGauntlet", NULL, 0, 0, &c) == GEAS_OK,
          "sign LangGauntlet");
    if (!c) {
    ...
    GeasValue e123[3] = { int_val(1), int_val(2), int_val(3) };
    ...
    GeasValue e45[2] = { int_val(4), int_val(5) };
    GeasValue find_hit[2] = { int_list(e45, 2), int_val(5) };
    memset(&out, 0, sizeof(out));
    CHECK(geas_pledge_fulfill_sync(c, "find", find_hit, 2, NULL, 0, &out) ==
              GEAS_OK,
          "find hit runs");
    CHECK(result_bool(&out, 0, 1), "find 5 in [4,5] is Ok(true)");
    GeasValue find_miss[2] = { int_list(e45, 2), int_val(6) };
    memset(&out, 0, sizeof(out));
    CHECK(geas_pledge_fulfill_sync(c, "find", find_miss, 2, NULL, 0, &out) ==
              GEAS_OK,
          "find miss runs");
    ...
    memset(&out, 0, sizeof(out));
    GeasValue oob = int_val(5);
    CHECK(geas_pledge_fulfill_sync(c, "pick", &oob, 1, NULL, 0, &out) ==
              GEAS_ERR_TYPE,
          "pick 5 out of bounds is GEAS_ERR_TYPE");
    memset(&out, 0, sizeof(out));
    GeasValue neg = int_val(-1);
    CHECK(geas_pledge_fulfill_sync(c, "pick", &neg, 1, NULL, 0, &out) ==
              GEAS_ERR_TYPE,
          "pick -1 is GEAS_ERR_TYPE through the unsigned cast");

    /* ---- head_doubled: both paths of '?' ---- */

    out = run1(c, "head_doubled", int_list(NULL, 0), "head_doubled []");
    CHECK(out.ty == GEAS_TY_OPTION && out.tag == 0,
          "head_doubled [] propagates None");
    GeasValue e21[1] = { int_val(21) };
    out = run1(c, "head_doubled", int_list(e21, 1), "head_doubled [21]");
    CHECK(out.ty == GEAS_TY_OPTION && out.tag == 1 && out.as.box &&
              ((const GeasValue*)out.as.box)->as.i == 42,
    ...
    GeasValue g303[3] = { int_val(0), int_val(5), int_val(0) };
     ...
     * GEAS_ERR_TYPE an eager xs[9] would raise. This is the short circuit
     * proof: an eager lowering faults here. */
    {
        GeasValue gargs[2] = { int_list(g303, 3), int_val(9) };
        memset(&out, 0, sizeof(out));
        CHECK(geas_pledge_fulfill_sync(c, "guard", gargs, 2, NULL, 0, &out) ==
                  GEAS_OK,
              "guard i=9 short circuits, no fault");
        CHECK(result_bool(&out, 0, 0), "guard i=9 is Ok(false), not GEAS_ERR_TYPE");
    }
    {
        GeasValue gargs[2] = { int_list(g303, 3), int_val(0) };
        memset(&out, 0, sizeof(out));
        CHECK(geas_pledge_fulfill_sync(c, "guard", gargs, 2, NULL, 0, &out) ==
                  GEAS_OK,
              "guard i=0 runs");
    ...
    {
        GeasValue gargs[2] = { int_list(g303, 3), int_val(1) };
        memset(&out, 0, sizeof(out));
        CHECK(geas_pledge_fulfill_sync(c, "guard", gargs, 2, NULL, 0, &out) ==
                  GEAS_OK,
              "guard i=1 runs");
    ...
    {
        GeasValue oargs[2] = { int_list(g303, 3), int_val(9) };
        memset(&out, 0, sizeof(out));
        CHECK(geas_pledge_fulfill_sync(c, "orguard", oargs, 2, NULL, 0, &out) ==
                  GEAS_OK,
    ...
    {
        GeasValue oargs[2] = { int_list(g303, 3), int_val(1) };
        memset(&out, 0, sizeof(out));
        CHECK(geas_pledge_fulfill_sync(c, "orguard", oargs, 2, NULL, 0, &out) ==
                  GEAS_OK,
              "orguard i=1 runs");
    ...
    memset(&out, 0, sizeof(out));
    CHECK(geas_pledge_fulfill_sync(c, "rec_copy", NULL, 0, NULL, 0, &out) ==
              GEAS_OK,
          "rec_copy runs");
    ...
    memset(&out, 0, sizeof(out));
    CHECK(geas_pledge_fulfill_sync(c, "list_copy", NULL, 0, NULL, 0, &out) ==
              GEAS_OK,
          "list_copy runs");
    CHECK(result_int(&out, 0, 199),
    ...
    memset(&out, 0, sizeof(out));
    CHECK(geas_pledge_fulfill_sync(c, "nested_copy", NULL, 0, NULL, 0, &out) ==
              GEAS_OK,
          "nested_copy runs");
    CHECK(result_int(&out, 0, 199),
    ...
    GeasValue clause_elems[3] = { int_val(1), int_val(2), int_val(3) };
    ...
    memset(&out, 0, sizeof(out));
    CHECK(geas_pledge_fulfill_sync(c, "map_copy", NULL, 0, NULL, 0, &out) ==
              GEAS_OK,
          "map_copy runs");
    CHECK(result_int(&out, 0, 199),
    ...
    memset(&out, 0, sizeof(out));
    GeasValue bw_oob = int_val(9);
    CHECK(geas_pledge_fulfill_sync(c, "board_write", &bw_oob, 1, NULL, 0,
                                  &out) == GEAS_ERR_TYPE,
          "board_write 9 out of range through the chain is GEAS_ERR_TYPE");
    ...
    memset(&out, 0, sizeof(out));
    GeasValue dw_miss = str_val("zed");
    CHECK(geas_pledge_fulfill_sync(c, "deep_write", &dw_miss, 1, NULL, 0,
                                  &out) == GEAS_ERR_TYPE,
    ...
    memset(&out, 0, sizeof(out));
    CHECK(geas_pledge_fulfill_sync(c, "snapshot", NULL, 0, NULL, 0, &out) ==
              GEAS_OK,
          "snapshot runs");
    CHECK(out.ty == GEAS_TY_RESULT && out.tag == 0 && out.as.box,
          "snapshot is Ok");
    if (out.ty == GEAS_TY_RESULT && out.tag == 0 && out.as.box) {
        const GeasValue* m = (const GeasValue*)out.as.box;
        ...
        char rbuf[128];
        size_t need = 0;
        CHECK(geas_value_render(m, rbuf, sizeof rbuf, &need) == GEAS_OK,
