 ...
 * geas_value_eq, the UInt high bit and a String with an embedded NUL included so
 ...
 *
 *   ./target/geas-out/test_store --emit tests/store
 *
...
/* ---- the bump arena behind the GeasStoreAlloc ---- */

/* One block, handed out with 16 byte alignment so a GeasValue array behind it
 ...
 * reclaims the lot, the S0 stand in for geas_bytes on an instance. */
...
static GeasValue str_bytes(const uint8_t* p, uint64_t n) {
...
static GeasValue str_cstr(const char* s) {
...
static void dump_field(Buf* w, const GeasValue* f) {
    switch (f->ty) {
    case GEAS_TY_INT:
...
static void dump_rows(Buf* w, const GeasValue* rows, const GeasString* names,
                      size_t ncols) {
    uint64_t nrows = rows->as.list.len;
    const GeasValue* rec = (const GeasValue*)rows->as.list.data;
    buf_fmt(w, "rows %llu\n", (unsigned long long)nrows);
    for (uint64_t r = 0; r < nrows; r++) {
        const GeasValue* fields = (const GeasValue*)rec[r].as.list.data;
...
static const uint32_t k_col_types[3] = { GEAS_TY_INT, GEAS_TY_FLOAT,
                                         GEAS_TY_STRING };

static GeasString col_name(const char* s) {
    GeasString a;
    a.ptr = (uint8_t*)s;
...
static int build_fixture(GeasStore* s, Arena* arena, Buf* w) {
    if (geas_store_exec(s, "CREATE TABLE t(id INTEGER, amt REAL, owner TEXT)") !=
        GEAS_OK) {
    ...
    GeasStoreAlloc alloc = { arena_bytes, arena };
    GeasString names[3] = { col_name("id"), col_name("amt"), col_name("owner") };
    GeasValue rows;
    if (geas_store_query(s, "SELECT id, amt, owner FROM t ORDER BY id", NULL, 0,
                        k_col_types, names, 3, &alloc, &rows) != GEAS_OK) {
 ...
 * geas_value_eq. The column is declared with no type so it holds each value's
 * own storage class exactly, the cleanest way to prove the bind and the read
 * are inverses for every scalar the store maps. */
static void round_trip(GeasStore* s, Arena* arena, uint32_t col_ty,
                       const GeasValue* v, const char* what) {
    if (geas_store_exec(s, "DELETE FROM rt") != GEAS_OK) {
        CHECK(0, "round trip: clear failed");
        return;
    }
    if (geas_store_exec_params(s, "INSERT INTO rt(v) VALUES(?)", v, 1, NULL) !=
        GEAS_OK) {
        CHECK(0, what);
        return;
    }
    GeasStoreAlloc alloc = { arena_bytes, arena };
    GeasValue rows;
    if (geas_store_query(s, "SELECT v FROM rt", NULL, 0, &col_ty, NULL, 1,
                        &alloc, &rows) != GEAS_OK) {
        CHECK(0, what);
        return;
    }
    ...
    const GeasValue* rec = (const GeasValue*)rows.as.list.data;
    const GeasValue* field = (const GeasValue*)rec[0].as.list.data;
    CHECK(geas_value_eq(v, &field[0]), what);
}

static void test_round_trips(GeasStore* s, Arena* arena) {
    if (geas_store_exec(s, "CREATE TABLE rt(v)") != GEAS_OK) {
        CHECK(0, "round trip: create rt failed");
        return;
    }

    GeasValue vi = int_val(-1234567890123LL);
    round_trip(s, arena, GEAS_TY_INT, &vi, "Int round trips");

    GeasValue vu = zeroed(GEAS_TY_UINT);
    vu.as.u = 0xDEADBEEFCAFEBABEULL; /* the high bit is set on purpose */
    round_trip(s, arena, GEAS_TY_UINT, &vu, "UInt round trips its bit pattern");

    GeasValue vf = float_val(-2.5);
    round_trip(s, arena, GEAS_TY_FLOAT, &vf, "Float round trips");

    GeasValue vb = zeroed(GEAS_TY_BOOL);
    vb.as.b = 1;
    round_trip(s, arena, GEAS_TY_BOOL, &vb, "Bool round trips");

    GeasValue vy = zeroed(GEAS_TY_BYTE);
    vy.as.b = 0xA5;
    round_trip(s, arena, GEAS_TY_BYTE, &vy, "Byte round trips");

    GeasValue vc = zeroed(GEAS_TY_CHAR);
    vc.as.ch = 0x1F600; /* a scalar value past the BMP */
    round_trip(s, arena, GEAS_TY_CHAR, &vc, "Char round trips");
    ...
    GeasValue vs = str_bytes(raw, 4);
    round_trip(s, arena, GEAS_TY_STRING, &vs, "String round trips raw bytes");
}
...
static uint64_t count_tx(GeasStore* s, Arena* arena) {
    GeasStoreAlloc alloc = { arena_bytes, arena };
    uint32_t ty = GEAS_TY_INT;
    GeasValue rows;
    if (geas_store_query(s, "SELECT id FROM tx ORDER BY id", NULL, 0, &ty, NULL,
                        1, &alloc, &rows) != GEAS_OK)
        return (uint64_t)-1;
    return rows.as.list.len;
}

static void test_transactions(GeasStore* s, Arena* arena) {
    if (geas_store_exec(s, "CREATE TABLE tx(id INTEGER)") != GEAS_OK) {
        CHECK(0, "tx: create failed");
        return;
    }
    GeasValue one = int_val(1);
    GeasValue two = int_val(2);

    CHECK(geas_store_begin(s) == GEAS_OK, "begin opens a transaction");
    CHECK(geas_store_exec_params(s, "INSERT INTO tx(id) VALUES(?)", &one, 1,
                                NULL) == GEAS_OK,
          "insert inside the transaction");
    CHECK(geas_store_commit(s) == GEAS_OK, "commit lands the write");
    CHECK(count_tx(s, arena) == 1, "a committed write is visible");

    CHECK(geas_store_begin(s) == GEAS_OK, "begin a second transaction");
    CHECK(geas_store_exec_params(s, "INSERT INTO tx(id) VALUES(?)", &two, 1,
                                NULL) == GEAS_OK,
          "insert to be rolled back");
    CHECK(geas_store_rollback(s) == GEAS_OK, "rollback discards the write");
    CHECK(count_tx(s, arena) == 1, "a rolled back write vanished");
}

/* ---- the negative corpus ---- */

static void test_negative(GeasStore* s, Arena* arena) {
    /* A dsn under a directory that does not exist cannot be created, so the
     * open fails cleanly rather than half opening. */
    GeasStore* bad = (GeasStore*)0x1;
    CHECK(geas_store_open("file:/nonexistent-geas-dir-9f3c/db.sqlite",
                         &bad) == GEAS_ERR_STORE,
          "a dsn that will not open is GEAS_ERR_STORE");
    CHECK(bad == NULL, "a failed open leaves the handle NULL");

    CHECK(geas_store_exec(s, "THIS IS NOT SQL") == GEAS_ERR_STORE,
          "broken SQL is GEAS_ERR_STORE");

    /* One placeholder, two parameters offered: the counts disagree. */
    GeasValue two_params[2] = { int_val(1), int_val(2) };
    CHECK(geas_store_exec_params(s, "INSERT INTO tx(id) VALUES(?)", two_params,
                                2, NULL) == GEAS_ERR_STORE,
          "a wrong parameter count is refused");

    /* A composite parameter has no place in a flat row. */
    GeasStoreAlloc alloc = { arena_bytes, arena };
    GeasValue list_param;
    memset(&list_param, 0, sizeof(list_param));
    list_param.ty = GEAS_TY_LIST;
    list_param.as.list.elem_ty = GEAS_TY_INT;
    CHECK(geas_store_exec_params(s, "INSERT INTO tx(id) VALUES(?)", &list_param,
                                1, NULL) == GEAS_ERR_TYPE,
          "a non scalar parameter is GEAS_ERR_TYPE");

    /* A non scalar result column is refused before a row is read. */
    uint32_t bad_col = GEAS_TY_LIST;
    GeasValue rows;
    CHECK(geas_store_query(s, "SELECT id FROM tx", NULL, 0, &bad_col, NULL, 1,
                          &alloc, &rows) == GEAS_ERR_TYPE,
          "a non scalar result column is GEAS_ERR_TYPE");
}
...
int main(int argc, char** argv) {
    ...
    GeasStore* s = NULL;
    int rc = 0;
    if (geas_store_open(dsn, &s) != GEAS_OK || !s) {
