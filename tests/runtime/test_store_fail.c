/* test_store_fail.c: the S3 failure gate, GEAS_ERR_STORE proven to be exactly
 ...
 * way a store can refuse the runtime, checking that each lands GEAS_ERR_STORE
 ...
 * loose insert is GEAS_ERR_STORE, and a transactional debit is GEAS_ERR_STORE
 ...
 * GEAS_ERR_STORE and distinct from the contract's own Err, and the table stands
 ...
 * so it surfaces GEAS_ERR_STORE rather than stalling forever, and both roll back.
 *
 ...
 * as a value with an GEAS_OK delivery, and it is never once GEAS_ERR_STORE, the
 ...
 * backend refusal returns GEAS_ERR_STORE here, the store failing the runtime. */
...
/* Fulfills one pledge synchronously, checks an GEAS_OK delivery, and hands back
...
static GeasContract* sign_dsn(GeasRuntime* rt, const char* dsn) {
    GeasVowBinding ovr = { "dsn", str_val(dsn) };
    GeasContract* c = NULL;
    GeasStatus st = geas_contract_sign(rt, "Ledger", &ovr, 1, 0, &c);
    CHECK(st == GEAS_OK && c && geas_contract_state(c) == GEAS_SIGNED,
          "sign Ledger");
    return c;
}
...
static double read_balance(GeasRuntime* rt, const char* dsn, int64_t id) {
    GeasContract* c = sign_dsn(rt, dsn);
    double bal = -1.0;
    if (c) {
...
int main(void) {
    ...
    GeasContract* seed = sign_dsn(rt, rw);
    if (seed) {
        GeasValue a1[3] = { int_val(1), str_val("alice"), float_val(100.0) };
        GeasValue a2[3] = { int_val(2), str_val("bob"), float_val(100.0) };
        CHECK(run_status(seed, "open", a1, 3) == GEAS_OK, "seed account 1");
        CHECK(run_status(seed, "open", a2, 3) == GEAS_OK, "seed account 2");
        geas_contract_break(seed);
    }

    /* ---- the business boundary: an overdraft is the ledger's own Err, a
     * value with an GEAS_OK delivery, and never a store status. This is the
    ...
    GeasContract* cb = sign_dsn(rt, rw);
    if (cb) {
        GeasValue over[2] = { int_val(1), float_val(500.0) };
        GeasValue out;
        memset(&out, 0, sizeof(out));
        GeasStatus st = geas_pledge_fulfill_sync(cb, "debit", over, 2, NULL, 0,
                                               &out);
        CHECK(st == GEAS_OK,
              "an overdraft delivers GEAS_OK, the contract working");
        CHECK(st != GEAS_ERR_STORE,
              "an overdraft is never GEAS_ERR_STORE, the boundary holds");
        CHECK(is_err(&out),
     ...
     * the second insert the schema's PRIMARY KEY forbids, GEAS_ERR_STORE, and it
     * is a store status and not a value, distinct from the ledger's own Err.
     * The table stands after the refusal. ---- */

    GeasContract* cc = sign_dsn(rt, rw);
    if (cc) {
        GeasValue dup[3] = { int_val(1), str_val("mallory"), float_val(7.0) };
        CHECK(run_status(cc, "open", dup, 3) == GEAS_ERR_STORE,
              "a duplicate primary key is GEAS_ERR_STORE");
        /* the table survives the refused insert: a fresh row still lands. */
        GeasValue fresh[3] = { int_val(3), str_val("carol"), float_val(5.0) };
        CHECK(run_status(cc, "open", fresh, 3) == GEAS_OK,
              "the table stands after a refused duplicate");
        geas_contract_break(cc);
    }
    CHECK(read_balance(rt, rw, 1) == 100.0,
     ...
     * write. A loose insert is GEAS_ERR_STORE. ---- */

    GeasContract* cro = sign_dsn(rt, ro);
    if (cro) {
        GeasValue w[3] = { int_val(9), str_val("dave"), float_val(1.0) };
        CHECK(run_status(cro, "open", w, 3) == GEAS_ERR_STORE,
              "a write to a read only connection is GEAS_ERR_STORE");

        /* a transactional debit against the read only file: the write the
         * episode buffers cannot land, GEAS_ERR_STORE, and the episode rolls
         * back so nothing half written survives. */
        GeasValue d[2] = { int_val(1), float_val(10.0) };
        CHECK(run_status(cro, "debit", d, 2) == GEAS_ERR_STORE,
              "a transactional write to a read only connection is GEAS_ERR_STORE");
        geas_contract_break(cro);
    }
    ...
    {
        GeasContract* cm = sign_dsn(rt, rw);
        if (cm) {
            GeasValue k9[1] = { int_val(9) };
            GeasValue r = run(cm, "balance", k9, 1);
            CHECK(is_err(&r), "the refused read only insert left no row 9");
            geas_contract_break(cm);
        }
    }
     ...
     * cannot acquire the file and surfaces GEAS_ERR_STORE rather than stalling
    ...
    GeasContract* hold = sign_dsn(rt, rw);
    GeasContract* race = sign_dsn(rt, rw);
    if (hold && race) {
        GeasValue d1[2] = { int_val(1), float_val(10.0) };
        CHECK(run_status(hold, "debit", d1, 2) == GEAS_OK,
              "the holder's debit opens the transaction");
        GeasValue d2[2] = { int_val(2), float_val(10.0) };
        CHECK(run_status(race, "debit", d2, 2) == GEAS_ERR_STORE,
              "the contended writer loses to the open transaction, GEAS_ERR_STORE");
    }
    if (race) geas_contract_break(race);
    if (hold) geas_contract_break(hold);
